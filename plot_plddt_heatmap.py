...
def find_boltz_json_files(root_dir):
    ...
    for predictions_dir in Path(root_dir).glob('*/boltz_results_*/predictions'):
...
def get_chai_identifier_from_path(file_path):
    """Extract identifier from CHAI file path."""
    # Get the path relative to OUTPUT/CHAI
    rel_path = file_path.relative_to(Path('OUTPUT/CHAI'))
...
def get_boltz_identifier_from_path(file_path):
    """Extract identifier from BOLTZ file path."""
    # Get the path relative to OUTPUT/BOLTZ
    rel_path = file_path.relative_to(Path('OUTPUT/BOLTZ'))
    parts = list(rel_path.parts)
...
def organize_data(chai_files, boltz_files):
    ...
    for file_path in chai_files:
        is_msa, subfolder = get_chai_identifier_from_path(file_path)
    ...
    for file_path in boltz_files:
        is_msa, subfolder = get_boltz_identifier_from_path(file_path)
...
def main():
    ...
    chai_files = find_chai_json_files(chai_dir)
    boltz_files = find_boltz_json_files(boltz_dir)
    ...
    data = organize_data(chai_files, boltz_files)
    
    # Create the heatmap
    output_file = 'plddt_heatmap.png'
    df = create_heatmap(data, output_file)
    
    # Save the data to CSV for further analysis
    csv_file = 'plddt_values.csv'
    df.to_csv(csv_file)
